...
Now imagine that same button receiving a prop called *:disabled* by any external condition and being turned opaque because of that, how to apply something like that? By transforming the __HEX__ into some __RGBa__ in order to use it's transparency hability? So let's talk about HSL, it's gonna be a blast for you.  
...
But shouldn't be easier? I mean, we're just talking about light getting in and out over a color when it comes to have it brighter or darker tone. But to get things done with both __HEX__ and __RGB__ we need to update __all the values in there__, getting a totally new code just in order to change the color brightness.
...
Can you see the readability that your code can get from it? Looks good, right?  
